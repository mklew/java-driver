...
public class DCAwareRoundRobinPolicyTest {
    ...
    @Test(groups="short", dataProvider="consistencyLevels", dataProviderClass = DataProviders.class)
        ...
        try {
            ...
            for (int i = 1; i <= 2; i++) {
                assertThat(queryTracker.queryCount(sCluster, 1, i)).isEqualTo(0);
                // then: Remote hosts should only be queried for non local CLs.
                assertThat(queryTracker.queryCount(sCluster, 2, i)).isEqualTo(expectedQueryCount);
            }
        } finally {
            cluster.close();
            sCluster.stop();
    ...
    public void should_use_remote_hosts_for_local_cl_when_allowed(ConsistencyLevel cl) {
        ...
        try {
            ...
            for (int i = 1; i <= 2; i++) {
                ...
                assertThat(queryTracker.queryCount(sCluster, 2, i)).isEqualTo(25);
            }
        } finally {
            cluster.close();
            sCluster.stop();
